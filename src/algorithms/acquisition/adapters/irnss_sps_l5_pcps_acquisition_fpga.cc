...
#include "irnss_sps_l5_pcps_acquisition_fpga.h"
...
IrnssSpsL5PcpsAcquisitionFpga::IrnssSpsL5PcpsAcquisitionFpga(
                                ...
                                out_streams_(out_streams)
{
    ...
    auto code_length = static_cast<uint32_t>(std::round(static_cast<double>(fs_in) / (IRNSS_L5I_CODE_RATE_CPS / IRNSS_L5I_CODE_LENGTH_CHIPS)));
    ...
    acq_parameters.excludelimit = static_cast<unsigned int>(1 + ceil(IRNSS_L5I_CHIP_PERIOD_S * static_cast<float>(fs_in)));
    ...
    for (uint32_t PRN = 1; PRN <= NUM_PRNs; PRN++)
        {
            irnss_sps_l5_code_gen_complex_sampled(code, PRN, fs_in, 0);  // generate PRN code
...
void IrnssSpsL5PcpsAcquisitionFpga::stop_acquisition()
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_threshold(float threshold)
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_doppler_max(unsigned int doppler_max)
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_doppler_step(unsigned int doppler_step)
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_doppler_center(int doppler_center)
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_gnss_synchro(Gnss_Synchro* gnss_synchro)
{
...
signed int IrnssSpsL5PcpsAcquisitionFpga::mag()
{
    return acquisition_fpga_->mag();
}


void IrnssSpsL5PcpsAcquisitionFpga::init()
{
    acquisition_fpga_->init();
}


void IrnssSpsL5PcpsAcquisitionFpga::set_local_code()
{
    acquisition_fpga_->set_local_code();
}


void IrnssSpsL5PcpsAcquisitionFpga::reset()
{
...
void IrnssSpsL5PcpsAcquisitionFpga::set_state(int state)
{
    acquisition_fpga_->set_state(state);
}


void IrnssSpsL5PcpsAcquisitionFpga::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void IrnssSpsL5PcpsAcquisitionFpga::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
...
gr::basic_block_sptr IrnssSpsL5PcpsAcquisitionFpga::get_left_block()
{
    return nullptr;
}


gr::basic_block_sptr IrnssSpsL5PcpsAcquisitionFpga::get_right_block()
{
    return nullptr;
}
